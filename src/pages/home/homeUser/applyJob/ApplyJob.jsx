...
import { postApplyJob } from "../../../../apis/jobs";
...
export default function ApplyJob({ position, company, id, close }) {
  ...
  const sendInfor = async () => {
    try {
      const result = await postApplyJob(infor);
      close(result.message, true);
    } catch (error) {
      close(error.response.message, false);
    }
  };
    ...
    <>
      <div className="applyJob__container">
        ...
        <div className="applyJob__content">
          <h2>
            Công ty {company?.name} tuyển dụng vị trí {position}
          </h2>
          <div className="applyJob__content__input">
            <input value={user?.name} type="text" placeholder="Tên của bạn" />
          </div>
          <div className="applyJob__content__inputImage">
            <p className="applyJob__content__inputImage__text">
            ...
            <div className="applyJob__content__inputImage__image">
              <div className="applyJob__content__inputImage__image--top">
                <div className="applyJob__content__inputImage__image--top__circle">
                  <div className="applyJob__content__inputImage__image--top__circle--inner"></div>
                </div>
              ...
              <div className="applyJob__content__inputImage__image--bottom">
                <p>
                  We accept .doc .docx, .pdf files, no password protected, up to
                  3MB
                </p>
              </div>
            </div>
            <div className="applyJob__content__inputImage__skill">
              <p className="applyJob__content__inputImage__skill__text">
                <strong>Mô tả bản thân</strong>
              </p>
              <p className="applyJob__content__inputImage__skill__textbot">
                Kĩ năng, các dự án bạn đã làm hoặc những điều bạn cảm thấy phù
                hợp với công việc này?
              </p>
              <textarea
                className="applyJob__content__inputImage__skill__textarea"
                placeholder="Kĩ năng, dự án..."
                name="content"
                id=""
                cols="58"
                rows="3"
                ...
                onChange={getChange}
              ></textarea>
              <p className="applyJob__content__inputImage__skill__textunder">
                Dưới 500 từ
              </p>
            </div>
          </div>
